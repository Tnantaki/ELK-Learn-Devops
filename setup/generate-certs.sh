...
openssl genrsa -out certs/elasticsearch-key.pem 2048

# Generate a Certificate Signing Request (CSR) for Elasticsearch
openssl req -new -key certs/elasticsearch-key.pem -out certs/elasticsearch.csr -subj "/C=TH/ST=None/L=Bangkok/O=42/CN=elasticsearch"
...
openssl genrsa -out certs/kibana-key.pem 2048

# Generate a Certificate Signing Request (CSR) for Kibana
openssl req -new -key certs/kibana-key.pem -out certs/kibana.csr -subj "/C=TH/ST=None/L=Bangkok/O=42/CN=kibana"
